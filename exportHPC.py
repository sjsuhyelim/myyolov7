...
from multi.multibackend import export_formats
...
def run(
    ...
    #model = attempt_load(weights, device=device, inplace=True, fuse=True)  # load FP32 model, different in yolov5
    ...
    shape = tuple(y[0].shape)  # model output shape, (1, 3, 80, 80, 9)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='data/waymococohpc.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default='../modelzoo/waymoyolov7trained/yolov5s.pt', help='model.pt path(s)')
